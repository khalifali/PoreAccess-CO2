...
\begin{frame}{Can inlet accessibility predict the fitted diffusivity?}

\begin{columns}[T,onlytextwidth]
\column{0.47\textwidth}\hi{A steady geometric descriptor}\small
\begin{itemize}
\item Use the same pore network with adsorption disabled.
\item Fix concentrations at the inlet and an interior plane.
\item Solve steady diffusion and sum the resulting molar flux $\dot n$.
\end{itemize}
\[
G_{\rm access}=\frac{\dot n}{\Delta c},\qquad
D_{\rm access}=\frac{G_{\rm access}H}{A_{\rm tube}}.
\]
$H$: bed height; $A_{\rm tube}$: tube cross-section.
\column{0.49\textwidth}\hi{Learn a transport closure}\small
\begin{itemize}
\item Pair each $D_{\rm access}$ with its fitted $D_{\rm eff}$.
\item Fit $D_{\rm eff}=f(D_{\rm access})$ on training beds.
\item Leave one bed out to test diffusivity prediction.
\item For a new bed, apply the closure and run the 1D model.
\end{itemize}
\end{columns}
\vspace{0.25cm}
\result{Accessibility measures steady gas access. The closure links it to transient uptake.}

\note{We return to the inlet-accessibility hypothesis. On each network we perform a separate steady diffusion calculation without adsorption, imposing a concentration difference from the inlet to a selected interior plane. Total molar flux divided by that difference defines the conductance. We multiply by full bed height divided by tube cross-section to form the diffusivity-scaled descriptor used in the code. This convention is distinct from measuring a local diffusivity over only the plane distance. It is not automatically equal to the fitted effective diffusivity: one describes steady access, the other reduces a transient adsorption problem. Their relationship is learned across beds. Leave-one-bed-out fitting uses nineteen beds to predict the remaining bed. This is internal validation in the current ensemble, not proof of universal transferability.}
\end{frame}
...
\note{This summarizes the two stages we have now defined. During development, the transient PNM supplies reference uptake curves. Per-bed optimization supplies fitted diffusivities, and steady access calculations supply the descriptors. We learn a relationship between them. For another packing within the validated scope, we calculate its steady accessibility, apply the closure and solve the reduced one-dimensional system. That avoids repeating transient PNM adsorption and per-bed optimization. We now examine the evidence for each link in this workflow.}
\end{frame}
...
\begin{frame}{Why not predict uptake directly from $G_{\rm access}$?}
  ...
  \end{columns}
  \vspace{0.6cm}
  \result{Endpoint correlation summarizes one time. A transport closure supplies a complete uptake history.}
\note{The results show a strong relationship between accessibility and final loading. If only ranking at the selected final time is needed, that relationship may already be useful. It does not by itself give a full uptake history or validate transfer to other conditions. A closure for the reduced diffusivity instead feeds a dynamic model that produces the whole curve and axial fields. This is why our main objective is a transport closure rather than only an endpoint regression.}
\end{frame}
...
\begin{frame}{Predictive performance across diagnostic depths}
  \begin{columns}[c,onlytextwidth]
...
% Optional detailed backup slides: uncomment the following line and compile twice.
