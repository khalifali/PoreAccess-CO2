...
\begin{frame}{}
  ...
  \end{FlexRow}
\note{Introduce the project: how random packing geometry can inform a reduced model of transient adsorption.}
\end{frame}

\begin{frame}{Presentation outline}
\tableofcontents
\note{We first motivate inlet accessibility, then explain the PNM reference and its mean uptake output. Next we reduce it to a homogeneous bed and develop the diffusivity closure. Results and conclusions follow.}
\end{frame}
...
\vfill
\note{Why should inlet-connected pathways matter?.}
\end{frame}
...
\note{The motivation is transport through random geometry. Two packings can have similar porosity but very different arrangements of constrictions and parallel pathways. In our experiment gas is supplied at the bottom and removed by adsorption as it enters. That makes the inlet a physically meaningful reference boundary. This leads us to test inlet-to-interior accessibility as a descriptor. It is a hypothesis, not a claim that the inlet alone controls every time scale. CO2 and zeolite provide a controlled application for studying this question.}
\end{frame}
