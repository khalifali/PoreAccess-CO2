...
\end{itemize}
\medskip
$\mathrm{CV}_{\varepsilon}=s_{\varepsilon}/\overline{\varepsilon}$
  \end{columns}
  \vspace{0.12cm}
...
\item compare the full uptake curve with the network
\item minimize time-weighted NRMSE
\end{enumerate}
\medskip
...
\begin{itemize}
\item affine accessibility model
\item proportional model
\end{itemize}
\medskip
The $5d_p$ plane gives the highest affine-model score in this ensemble.
  \end{columns}
  \vspace{0.12cm}
  {\small\result{The affine accessibility model remains predictive from $3d_p$ to $10d_p$.}}
  \note{This plot compares affine and proportional closures at each depth. It does not plot the power-law score. Five particle diameters gives the best affine-model score among these tested depths. This sensitivity supports the association while independent beds are still needed to evaluate model and depth selection.}
\end{frame}
