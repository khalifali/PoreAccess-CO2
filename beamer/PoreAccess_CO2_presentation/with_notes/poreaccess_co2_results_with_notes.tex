...
\begin{frame}{Twenty beds isolate variability caused by random arrangement}
  \begin{FlexRow}{0.57\textheight}
    ...
    \Col{0.53}{
      ...
      \begin{itemize}
        \item same zeolite beads and tube, only the insertion seed changes
        \item dry, isothermal gas supplied from a reservoir below the bed
        \item interparticle diffusion and bead adsorption, with no imposed flow
        \item heat effects and competitive water adsorption are outside this model
      \end{itemize}}
  \end{FlexRow}
...
\note{We isolate arrangement effects using twenty packings of identical zeolite beads in the same tube. Temperature, inlet concentration and adsorption parameters are fixed. The gas is dry and isothermal, with no imposed through-flow. The concentration is approximately equivalent to fifteen volume percent CO2 at atmospheric pressure and 298 K. This is a controlled diffusion-driven adsorption problem, not an industrial breakthrough simulation. The next step is to turn the particle geometry into a pore network.}
\end{frame}
...
\note{The particle centres define Delaunay tetrahedra. Valid circumcentres become pore nodes, and a shared triangular face defines a candidate throat between neighbouring nodes. The throat conductance follows the molecular diffusivity times an approximate opening area divided by length. Sobol sampling assigns gas volumes to nodes, while connectivity comes from geometry. This is the construction summary; the detailed sketches remain in the optional appendix. Now we have storage volumes and connections and can write a gas balance for every pore.}
\end{frame}
