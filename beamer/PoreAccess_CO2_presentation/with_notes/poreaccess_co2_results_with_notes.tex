...
\begin{frame}{Nearly equal porosity produced clearly different uptake curves}
  ...
  \note{
    Panel a shows that the twenty beds have closely matched bulk porosity. The
    mean is approximately 0.44665. The sample standard deviation divided by
    this mean gives a coefficient of variation of only 0.55 percent.

    Panel b shows a different picture for adsorption. The grey lines represent
    all twenty network uptake curves. The highlighted curves correspond to
    beds with low, median and high accessibility. At 5000 seconds, the mean
    loading ranges from about 0.206 to 0.315 moles per kilogram, producing a
    coefficient of variation of 11 percent. Thus, closely matched void volume
    does not produce closely matched transient utilization. Something about
    the arrangement and connectivity of that void space must explain the
    difference.
  }
\end{frame}
...
\begin{frame}{Accessibility explains the response that porosity misses}
  ...
  \note{
    Panel a confirms that bulk porosity has almost no monotonic relationship
    with final uptake. Panel b shows a strong positive relationship between
    accessibility conductance and uptake. The Spearman coefficient is 0.968,
    meaning that beds ranked as more accessible generally also rank higher in
    final uptake.

    Panel c examines local particle utilization. We define a particle as
    underutilized when its loading after 5000 seconds remains below half of the
    equilibrium loading corresponding to the inlet pressure. The symbol
    $\mathbb{I}$ is an indicator function. It equals one when this condition is
    true and zero otherwise. Averaging the indicator over all particles gives
    the underutilized fraction. Higher $G_{\rm access}$ corresponds to fewer
    underutilized particles. The descriptor therefore relates not only to the
    mean uptake but also to how effectively the bed interior is reached.
  }
\end{frame}
...
\begin{frame}{For each bed, $D_{\rm eff}$ is fitted to the complete uptake curve}
  ...
  \vspace*{0.2cm}
  \small
  Solid lines: pore-network reference $\overline q_{\rm PNM}(t)$\quad
  ...
  \vspace{-0.06cm}
  \small Panel (b) shows the residual $\overline q_{\rm 1D}-\overline q_{\rm PNM}$:
  ...
  \note{
    This slide shows the central reduction step. For each bed, we first run the
    pore-network model once and obtain its reference uptake curve. We then
    start the 1D model with a trial diffusivity $D$. The 1D solver produces a
    complete uptake curve, which we compare with the network curve. An
    optimization routine selects another $D$ and repeats the transient 1D solve.
    The process continues until the NRMSE reaches its minimum. The minimizing
    value is the fitted $D_{\rm eff}$ for that bed.

    In panel a, the solid lines are pore-network curves and the dashed lines are
    the optimized 1D curves. Panel b plots
    $\overline q_{\rm 1D}-\overline q_{\rm PNM}$. Positive values indicate that
    the 1D model predicts too much uptake, and negative values indicate too
    little. NRMSE is the root-mean-square of this residual over time, normalized
    by the final network loading. It summarizes the mismatch of the complete
    curves, not only their final points. Across the twenty beds, the mean NRMSE
    is 1.58 percent, so one optimized coefficient reproduces each detailed
    network response closely.
  }
\end{frame}
...
\begin{frame}{The closure maps a cheap descriptor to the fitted coefficient}
  ...
  \note{
    After the previous optimization, every bed has one fitted
    $D_{\rm eff}$ and one independently calculated $D_{\rm access}$. We now
    develop a closure, meaning an equation that predicts the reduced-model
    coefficient from a quantity available from the packing.

    We compare three equations. The first predicts $D_{\rm eff}$ from porosity.
    The second assumes direct proportionality to $D_{\rm access}$. The third is
    a positive dimensionless power law. Panel a shows the selected power-law
    relationship. The Spearman coefficient $\rho_s$ measures monotonic rank
    association, while the $p$ value tests the hypothesis of no monotonic
    association. Panel b contains held-out predictions from all three models,
    including the proportional model shown in orange.

    We use leave-one-bed-out cross-validation. Each bed is predicted by a
    closure fitted only to the other nineteen beds. The power law gives
    $R^2=0.890$, whereas the porosity model gives a negative $R^2$ and has no
...
\begin{frame}{The conclusion is stable from $3d_p$ to $10d_p$}
  ...
  \note{
    We repeated the accessibility calculation using planes at three, five,
    seven and ten particle diameters from the inlet. The left panel shows that
    both Pearson and Spearman correlations with fitted $D_{\rm eff}$ remain
    high at all four depths. The right panel shows the corresponding
    leave-one-bed-out prediction scores.

    The result is therefore not created by one arbitrary plane position. We
    retain five particle diameters because this plane lies beyond the immediate
    inlet layer, includes second-layer and deeper bottlenecks, and gives the
    strongest prediction in this data set. The proportional closure is weaker
    than the affine accessibility model at every tested depth, which also
    supports using a fitted nonlinear closure rather than identifying
    $D_{\rm access}$ directly with $D_{\rm eff}$.
  }
\end{frame}
