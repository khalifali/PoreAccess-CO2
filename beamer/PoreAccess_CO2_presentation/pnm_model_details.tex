...
\begin{frame}{Particle uptake: the prescribed LDF coefficient}
\begin{columns}[T,onlytextwidth]
\column{0.49\textwidth}\small
\hi{Literature input and conversion}
\[
\frac{D_{\rm particle}}{R_p^2}=5.7\times10^{-4}\ \mathrm{s^{-1}}
\]
{\footnotesize Commercial 13X beads; CO$_2$ in N$_2$,\\38$^\circ$C and 0.1 bar (Hu et al.).\par}
\medskip
Spherical-particle LDF approximation:
\[
k_{\rm LDF}\approx15\:\frac{D_{\rm particle}}{R_p^2}
\]
\[
\boxed{k_{\rm LDF}=8.55\times10^{-3}\ \mathrm{s^{-1}}}
\]
\column{0.47\textwidth}\small
\hi{Meaning in our model}
\[
\frac{dq_k}{dt}=k_{\rm LDF}(q_k^*-q_k)
\]
\begin{itemize}\setlength{\itemsep}{4pt}
\item One prescribed coefficient in both PNM and 1D models.
\item Represents uptake within a bead; distinct from bed-scale $D_{\rm eff}$.
\item At a fixed equilibrium target: relaxation time $1/k_{\rm LDF}\approx117$ s.
\end{itemize}
\end{columns}
\medskip
{\footnotesize\color{tumdark}\textbf{Assumption:} the 38$^\circ$C literature value is used unchanged at 298.15 K (25$^\circ$C). The factor-15 conversion is ours, not a directly reported LDF coefficient.\par}
\source{Hu et al., Adsorption 20 (2014), 121--135. \href{https://doi.org/10.1007/s10450-013-9554-z}{doi:10.1007/s10450-013-9554-z}. Parameter provenance: project reproducibility notes, Sec. 9.5.}
\note{The LDF coefficient is prescribed, not fitted separately for each bed. Our parameter provenance records a particle diffusion-rate parameter D over radius squared of 5.7 times ten to the minus four per second from Hu et al. for commercial zeolite 13X beads near our 0.8 mm radius. Using the spherical-particle approximation, we multiply this rate parameter by 15 to obtain k LDF equal to 8.55 times ten to the minus three per second. This is our conversion of the literature parameter, not a directly measured LDF coefficient. At a constant local equilibrium target, the difference between equilibrium and actual loading decays exponentially with a characteristic time of about 117 seconds. That is not the equilibration time of the entire bed: gas supply and the evolving local equilibrium target also matter. Both the PNM and reduced 1D model use the same coefficient. The source conditions are 38 degrees Celsius and 0.1 bar, while our model runs at 25 degrees Celsius. We adopt the coefficient without a temperature correction, which is an explicit modelling assumption.}
\end{frame}
